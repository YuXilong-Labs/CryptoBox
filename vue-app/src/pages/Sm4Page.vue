...
// SM4 pure JS implementation (ECB mode)
...
function tau(A) { return (SBOX[(A>>>24)&0xff]<<24 | SBOX[(A>>>16)&0xff]<<16 | SBOX[(A>>>8)&0xff]<<8 | SBOX[A&0xff])>>>0 }
function L(B) { return (B ^ rotl(B,2) ^ rotl(B,10) ^ rotl(B,18) ^ rotl(B,24))>>>0 }
function Lprime(B) { return (B ^ rotl(B,13) ^ rotl(B,23))>>>0 }
function T(A) { return L(tau(A)) }
function Tprime(A) { return Lprime(tau(A)) }

function sm4KeyExpand(key) {
  const MK = new Uint32Array(4)
  for (let i=0;i<4;i++) MK[i] = (key[i*4]<<24|key[i*4+1]<<16|key[i*4+2]<<8|key[i*4+3])>>>0
  const K = new Uint32Array(36)
  for (let i=0;i<4;i++) K[i] = (MK[i]^FK[i])>>>0
  const rk = new Uint32Array(32)
  for (let i=0;i<32;i++) { rk[i] = (K[i] ^ Tprime((K[i+1]^K[i+2]^K[i+3]^CK[i])>>>0))>>>0; K[i+4]=rk[i] }
  return rk
}

function sm4Block(input, rk) {
  const X = new Uint32Array(36)
  for (let i=0;i<4;i++) X[i] = (input[i*4]<<24|input[i*4+1]<<16|input[i*4+2]<<8|input[i*4+3])>>>0
  for (let i=0;i<32;i++) X[i+4] = (X[i] ^ T((X[i+1]^X[i+2]^X[i+3]^rk[i])>>>0))>>>0
  const out = new Uint8Array(16)
  for (let i=0;i<4;i++) { const w = X[35-i]; out[i*4]=w>>>24; out[i*4+1]=(w>>>16)&0xff; out[i*4+2]=(w>>>8)&0xff; out[i*4+3]=w&0xff }
  return out
}
...
const input = ref('')
const output = ref('')
const { save: saveHistory, getLast: getLastInput } = useHistory('sm4')
...
function encrypt() {
  try {
    const keyBytes = hexToBytes(key.value)
    const rk = sm4KeyExpand(keyBytes)
    ...
    padded.set(plainBytes); padded.fill(padLen, plainBytes.length)
    const result = new Uint8Array(padded.length)
    for (let i = 0; i < padded.length; i += 16) {
      result.set(sm4Block(padded.subarray(i, i+16), rk), i)
    }
...
function decrypt() {
  try {
    const keyBytes = hexToBytes(key.value)
    const rk = sm4KeyExpand(keyBytes)
    const rkDec = new Uint32Array(32); for(let i=0;i<32;i++) rkDec[i]=rk[31-i]
    const cipherBytes = hexToBytes(input.value)
    const result = new Uint8Array(cipherBytes.length)
    for (let i = 0; i < cipherBytes.length; i += 16) {
      result.set(sm4Block(cipherBytes.subarray(i, i+16), rkDec), i)
    }
    const padLen = result[result.length-1]
    output.value = new TextDecoder().decode(result.subarray(0, result.length - padLen))
  } catch(e) { output.value = `错误: ${e.message}` }
}
</script>
...
<template>
  <div>
    <div class="mb-6">
      ...
      <p class="text-[var(--text-3)] text-[12px] mt-1 ml-[42px]">SM4 对称加密 (ECB 模式, 128-bit key, PKCS7 填充)</p>
    </div>

    <div class="flex flex-col gap-1 mb-4 max-w-md">
      <label class="text-[11px] text-[var(--text-3)] uppercase tracking-wide font-medium">KEY (128-bit Hex)</label>
      <div class="relative flex items-center">
        <input v-model="key" placeholder="16 bytes hex (32 chars)" class="!pr-8" />
    ...
    <IOPanel v-model:inputValue="input" :outputValue="output" @clear="input = ''" />
