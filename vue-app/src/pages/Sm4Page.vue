<script setup>
import { ref, onMounted } from 'vue'
import IOPanel from '../components/IOPanel.vue'
import CopyBtn from '../components/CopyBtn.vue'
import { useHistory } from '../composables/useHistory.js'

// SM4 pure JS (ECB mode)
const SBOX = []
const CK = [0x00070e15,0x1c232a31,0x383f464d,0x545b6269,0x70777e85,0x8c939aa1,0xa8afb6bd,0xc4cbd2d9,0xe0e7eef5,0xfc030a11,0x181f262d,0x343b4249,0x50575e65,0x6c737a81,0x888f969d,0xa4abb2b9,0xc0c7ced5,0xdce3eaf1,0xf8ff060d,0x141b2229,0x30373e45,0x4c535a61,0x686f767d,0x848b9299,0xa0a7aeb5,0xbcc3cad1,0xd8dfe6ed,0xf4fb0209,0x10171e25,0x2c333a41,0x484f565d,0x646b7279]
const FK = [0xa3b1bac6,0x56aa3350,0x677d9197,0xb27022dc]

function rotl(x,n) { return ((x<<n)|(x>>>(32-n)))>>>0 }
function tau(A) { return (SBOX[(A>>>24)&0xff]<<24 | SBOX[(A>>>16)&0xff]<<16 | SBOX[(A>>>8)&0xff]<<8 | SBOX[A&0xff])>>>0 }
function L(B) { return (B ^ rotl(B,2) ^ rotl(B,10) ^ rotl(B,18) ^ rotl(B,24))>>>0 }
function Lprime(B) { return (B ^ rotl(B,13) ^ rotl(B,23))>>>0 }
function T(A) { return L(tau(A)) }
function Tprime(A) { return Lprime(tau(A)) }

function sm4KeyExpand(key) {
  const MK = new Uint32Array(4)
  for (let i=0;i<4;i++) MK[i] = (key[i*4]<<24|key[i*4+1]<<16|key[i*4+2]<<8|key[i*4+3])>>>0
  const K = new Uint32Array(36)
  for (let i=0;i<4;i++) K[i] = (MK[i]^FK[i])>>>0
  const rk = new Uint32Array(32)
  for (let i=0;i<32;i++) { rk[i] = (K[i] ^ Tprime((K[i+1]^K[i+2]^K[i+3]^CK[i])>>>0))>>>0; K[i+4]=rk[i] }
  return rk
}

function sm4Block(input, rk) {
  const X = new Uint32Array(36)
  for (let i=0;i<4;i++) X[i] = (input[i*4]<<24|input[i*4+1]<<16|input[i*4+2]<<8|input[i*4+3])>>>0
  for (let i=0;i<32;i++) X[i+4] = (X[i] ^ T((X[i+1]^X[i+2]^X[i+3]^rk[i])>>>0))>>>0
  const out = new Uint8Array(16)
  for (let i=0;i<4;i++) { const w = X[35-i]; out[i*4]=w>>>24; out[i*4+1]=(w>>>16)&0xff; out[i*4+2]=(w>>>8)&0xff; out[i*4+3]=w&0xff }
  return out
}

const key = ref('')
const input = ref('')
const output = ref('')
const { save: saveHistory, getLast: getLastInput } = useHistory('sm4')
onMounted(() => { input.value = getLastInput() })

function randomHex(n) { return Array.from(crypto.getRandomValues(new Uint8Array(n))).map(b=>b.toString(16).padStart(2,'0')).join('') }
function hexToBytes(h) { const b=new Uint8Array(h.length/2); for(let i=0;i<h.length;i+=2) b[i/2]=parseInt(h.substr(i,2),16); return b }

function encrypt() {
  try {
    const keyBytes = hexToBytes(key.value)
    const rk = sm4KeyExpand(keyBytes)
    const plainBytes = new TextEncoder().encode(input.value)
    const padLen = 16 - (plainBytes.length % 16)
    const padded = new Uint8Array(plainBytes.length + padLen)
    padded.set(plainBytes); padded.fill(padLen, plainBytes.length)
    const result = new Uint8Array(padded.length)
    for (let i = 0; i < padded.length; i += 16) result.set(sm4Block(padded.subarray(i, i+16), rk), i)
    output.value = Array.from(result).map(b=>b.toString(16).padStart(2,'0')).join('')
  } catch(e) { output.value = `错误: ${e.message}` }
}

function decrypt() {
  try {
    const keyBytes = hexToBytes(key.value)
    const rk = sm4KeyExpand(keyBytes)
    const rkDec = new Uint32Array(32); for(let i=0;i<32;i++) rkDec[i]=rk[31-i]
    const cipherBytes = hexToBytes(input.value)
    const result = new Uint8Array(cipherBytes.length)
    for (let i = 0; i < cipherBytes.length; i += 16) result.set(sm4Block(cipherBytes.subarray(i, i+16), rkDec), i)
    const padLen = result[result.length-1]
    output.value = new TextDecoder().decode(result.subarray(0, result.length - padLen))
  } catch(e) { output.value = `错误: ${e.message}` }
}

function swap() { const t = input.value; input.value = output.value; output.value = t }
</script>

<template>
  <div>
    <div class="tool-header fade">
      <h1>SM4 国密</h1>
      <p>SM4 对称加密 (ECB 模式, 128-bit key, PKCS7 填充)</p>
    </div>

    <div class="key-bar fade d1">
      <div class="key-group">
        <span class="cfg-label">Key</span>
        <div class="key-input-wrap">
          <input class="key-input" v-model="key" placeholder="16 bytes hex (32 chars)" />
          <CopyBtn :value="key" class="key-copy-btn" />
        </div>
        <button class="qbtn" @click="key = randomHex(16)">🎲 随机</button>
      </div>
    </div>

    <IOPanel v-model:inputValue="input" :outputValue="output" @clear="input = ''" @swap="swap">
      <template #config>
        <div class="cc-group">
          <span class="cc-label" style="font-size:11px;letter-spacing:0">ECB · PKCS7 · Hex</span>
        </div>
      </template>
      <template #actions>
        <button class="ca-btn primary" @click="encrypt"><span class="btn-text">加密</span></button>
        <button class="ca-btn" @click="decrypt"><span class="btn-text">解密</span></button>
      </template>
    </IOPanel>
  </div>
</template>
